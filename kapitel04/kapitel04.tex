...
First, the expression of the outage constraint on the uncertain interference is determined as 
\begin{align}
...
\end{IEEEproof}
Similar to the analysis performed in Corollary \ref{cor_US:cor1} for the deterministic channel, the performance bound $(\snrrcvdu^*)$ in terms of $\snrrcvdu$ and $\test$ for the random channel is evaluated. To this end, $\snrrcvdu^*$ is determined by substituting $\preg$ with $\pc$ in the expression (\ref{eq_US:preg_fad}) 
\begin{align}
%\hspace{-4mm}
...
\normalfont
\figurename~\ref{fig_US:or_fad} analyzes the variation of $\snrrcvdu^*$ with $\test$ for different $m \in \{0.5, 1, 2, 5, \infty\}$, where $m = \infty$ represents a deterministic channel. It is observed that $\snrrcvdu^*$ attains a lower value as the channel fading becomes severe, thereby enabling the US to operate at low $\snrrcvdu$ by extending the interference-limited regime. Following the analysis from Remark \ref{rm_US:rm1}, this also reflects that the power control becomes proactive as the severity in the fading increases. This signifies that controlled power employed for regulating the uncertainty interference is mainly dominated by the variations due to the channel fading, specially for the scenarios that observe severe fading. In addition, it is noticed that the deterministic channel is more sensitive to the estimation time as compared to the random channels.  
\end{remark}
After determining the controlled power in Lemma \ref{lm_US:lm6} that regulates the uncertain interference, the expression of the secondary throughput is determined.
\begin{align}
...
It can be depicted that the estimation-throughput tradeoff yields a suitable estimation time $\ttest$ that results in an optimum secondary throughput $\trs(\ttest)$. Hereafter, theoretical expressions are considered for the performance analysis with respect to the deterministic channel. In addition, the US operates at the suitable estimation time. 
