...
%% Calculate frequency response for Simulink
f = linspace(0,upper_frequency,values+1);

if(exist('filter_out','var'))
    
    imp_resp = (filter_out(1,:));
    ind = find(filter_out>0);
    imp_resp = imp_resp(ind(1):ind(length(ind)));
    
    freq_resp = zeros(1,values);
    for i = 1:length(f)
        freq_resp(i) = fourier_trafo(imp_resp,T_DAC,f(i));
    end
    [val,pos] = max(abs(freq_resp));
    freq_resp = [fliplr(freq_resp(2:length(freq_resp))),freq_resp]/val;
end
...
discrete_time_resp = downsample(discrete_time_resp,D);

discrete_freq_resp = zeros(1,values);
for i = 1:length(f)
    discrete_freq_resp(i) = fourier_trafo(discrete_time_resp.',D/(US_FIR*f_s),f(i));
end
...
% CIC Interpolator
cic_f = ((sin(pi*f*M_CIC/f_s*D/US_FIR)./sin(pi*f/f_s*D/US_FIR/US_CIC)).^N_CIC)/(US_CIC^N_CIC);
...
print -dpdf ..\figures\tetra_usrp_resampling.pdf
