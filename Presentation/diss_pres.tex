...
\pnote{Although these technologies contributes to an increase in data rate, this large number in data rate cannot be achieved without an extension to already allocated spectrum}
...
\pnote{Next, different spectrum access, which primarily constitutes the CR communication between the CSC-BS and the MS represents a user interface, wireless backhaul between CSC-BS and MS meant for exchanging the control and user information} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{Secondary access is enabled through the employment of CR techniques at the ST. Through the implementation of these techniques, a CR system guarantees sufficient protection at the PR in the form of interference power received at the PR} 
	...
	\pnote{Incorporating channel knowledge in the system model allows us to analyze the performance of CR system in conditions which are one step closer to reality} 
	\vspace{-3mm}
	\fs{7}{8}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{In order to validate the exprerrions derrived made while establishing the system model, a hardware is deployed using a software-defined platform.}
	\pnote{Finally, a applicability of the propositions (time allocation, channel estimation, variations) made are verified by means of a hardware demonstrator}  
	\vspace{-1.5mm}
	\begin{columns}
		\begin{column}[t]{0.53\columnwidth}
			\centering Challenges: \\[-0.2em]
				...
				\begin{block}{\scriptsize Hardware Feasibility?}%{\scriptsize Channel knowledge}
					\begin{itemize}
					\item Validatity of the expressions obtained? 
					\item Applicability of the proposiitions under realistic scenarios? 
					\end{itemize}	
				\end{block}
				}
		\end{column}
