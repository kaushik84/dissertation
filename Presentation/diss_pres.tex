		...
		\begin{center}
			...
			{\color{blue}{PT, PR}}, \color{red}{ST($\Leftrightarrow$ CSC-BS) and SR($\Leftrightarrow$ MS)}\\\vspace{0.2cm} 
				...
				\only<3->
				{
					...
					\fill[opacity = 0.2, fill = blue] (3.38,1.86) rectangle (3.78,2.22);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{By listening to signal transmitted by the PT, the ST detects the presence and absence of primary user signal. In thsi way, the ST protects the PR, by not transmitting its own signal whenever the primary user signal is detected.} 
	...
	\begin{columns}
		...
		\begin{center}
			...
			%Interweave Scenario \\\vspace{0.2cm} 
				...
				\only<3>
				{
					\fill[opacity = 0.2, fill = orange] (1.03,1.41) rectangle (1.43,1.75);
					\fill[opacity = 0.2, fill = blue] (1.35,0.2) rectangle (1.75,.54);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{The derived expressions are used to determine the suitable estimation and sensing time intervals that achieves the maximum secondary throughput.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{Unlike interweave system that considers temporal separation, the underlay system allows the secondary users to co-exist during the same time interval however separated in space, only if the interference received at the PR is kept below a desired level.}
	\pnote{A power control mechanism is employed at the ST}
	\pnote{Now, to employ power control, the ST listens to signals transmitted by the PR and apply channel reciprocity so as to maintain the interference received at PR is below a certain threshold.} 
	...
	\begin{block}{}%{\scriptsize Power control} 
		...
		\begin{align*}
			...
			\begin{cases} 
				...
				\end{cases} \\
				\text{where  } \apo &= \frac{{\color{kit-green100}{\test \fsam}} (1 + {\color{kit-green100}{\snrrcvdu}})^2}{2 + 4 {\color{kit-green100}{\snrrcvdu}}} \text{ and } \bpo = \frac{\nps (2 + 4 {\color{kit-green100}{\snrrcvdu}})}{{\color{kit-green100}{\test \fsam}} (1 + {\color{kit-green100}{\snrrcvdu}})},
			\end{align*}
	\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	{	
		...
		\begin{itemize}
			...
			\item Below $\test$, the time resources are used for reducing the variations via control power		
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{The modified version of the deployment scenario and the frame structure is illustrated in the figure}
	\pnote{The performance parameter, the interference received at the PR and the throughput at SR are presented, followed by the estimation throughput tradeoff subject to confidence probability constraint}
	%%\vspace{-3mm}
	\fs{7}{8}
