...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\pnote{Unlink interweave system, which employ time interlacing, the underlay system allow the secondary users to co-exist at the same time only if the interference received at the PR is kept below a desired level.}
	\pnote{A power control mechanism is employed at the ST that listens to signals transmitted by the PR. In this regard, secondary listens interchangeably to the transmissions from the PT and PR.}
	\pnote{} 
	\vspace{-4mm}
	\fs{7}{8}
	\begin{columns}
		...
		\begin{center}
			...
			%Interweave Scenario \\\vspace{0.2cm} 
				...
				\only<1>
				{
					\fill[opacity = 0.2, fill = kit-green100] (1.0,1.63) rectangle (1.4,1.97);
					\fill[opacity = 0.2, fill = kit-green100] (2.12,0.54) rectangle (2.52,.88);
			...
			\begin{tikzpicture}[scale=1]
				...
				\fill[opacity = 0.4, fill = kit-green100] (0.77,2.21) rectangle (1.41,2.54);
				\fill[opacity = 0.4, fill = kit-green100] (3.63,1.01) rectangle (4.27,1.34);
			\end{tikzpicture}	
		\end{center}
		\end{column}
	\end{columns}
