...
if rs_SNR
    ...
    h = figure(1);
    plot(SNR_dB(1,:),rs(1,:),'b','LineWidth',1);
    hold on;
    plot(SNR_dB(2,:),rs(2,:),'b--','LineWidth',1);
    plot(SNR_dB(3,:),rs(3,:),'b-.','LineWidth',1);
...
if test_SNR
    load('test_rs_SNR_results');
    h = figure(2);
    plot(SNR_dB(1,:),tau(1,:),'b','LineWidth',1)
    hold on
    plot(SNR_dB(2,:),tau(2,:),'b--','LineWidth',1)
    plot(SNR_dB(3,:),tau(3,:),'b-.','LineWidth',1)
    xlabel('$\snrrcvd = [\SI{}{dB}]$', 'FontSize', Fontsize);
