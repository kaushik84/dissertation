...
Since the stochastic model is the basis of the performance analysis of the proposed framework. It is reasonable to first validate the pdfs of the system variables $\eprcvd$, $\epreg$, $\eprcvdpr$, and $\ers$ derived in Section \ref{model}. To this end, measurements with the setup illustrated in Fig. \ref{messaufbau} is performed. The measurement data is plotted in terms of histograms and scaled such that it represents the relative frequency ($f_\textrm{hist}[x]$), which are evaluated over a certain set of bins $x \in \mathcal X_\text{bins}$. \figurename~\ref{hrel_pdf} compares the histograms from the measurements and pdfs determined using the analytical expressions for a certain set system parameters depicted in Table \ref{params}. The plots show that the theoretical expressions accurately capture the performance of the CR systems operating under realistic situations. 
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1]
                \node[anchor=south west,inner sep=0] (image) at (0,0)
        	{
			\includegraphics[width=0.5\textwidth]{figures/P_ST_Rx} 
                };
	...
	\includegraphics[width=0.5\textwidth]{figures/P_ST_Tx} \\[6pt] %
	
	\input{figures/P_PR_Rx}
	\includegraphics[width=0.5\textwidth]{figures/P_PR_Rx} \\[6pt]%
	\input{figures/R_s}
	\includegraphics[width=0.5\textwidth]{figures/R_s}%
	...
	\caption{Theoretical expressions of the pdf and experimental results with the system parameters presented in Table \ref{param}.}
	\label{hrel_pdf}
	%\vspace{-10 pt}
\end{figure}

The experiments were repeated for different values of $\snrrcvd$. It was observed that for a considerable range of $\snrrcvd \in (4,30)\SI{}{dB}$, the theoretical expressions depicted a significant accuracy to the experimental data, refer to Table \ref{nichtzentral}. The accuracy was quantified in terms of relative error ($\epsilon$) defined as
\begin{equation}
\label{fr}
\epsilon = \frac{1}{|\mathcal X\sub{bins}|} \times \sum_{x \in \mathcal X\sub{bins}} \frac{\dprcvd(x) - f\sub{hist}[x]}{f\sub{hist}[x]} \;  , 
\end{equation}
$|\mathcal X\sub{bins}|$ represents the cardinality of $\mathcal X\sub{bins}$, which excludes bins with $f\sub{hist}[x] \neq 0$. 
...
Based on this validation described in this section, following conclusions can be outlined: 
\begin{itemize}
