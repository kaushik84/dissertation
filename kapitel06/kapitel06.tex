...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1]
		...
		\begin{scope}[x={(image.south east)},y={(image.north west)}]
                \node[above, font=\footnotesize] at (0.13,0.92) {$ \times 10^{-9}$};
	...
	\begin{tikzpicture}[scale=1]
                \node[anchor=south west,inner sep=0] (image) at (0,0)
        	{
			\includegraphics[width=0.5\textwidth]{figures/P_ST_Tx} 
                };

		\begin{scope}[x={(image.south east)},y={(image.north west)}]
                \node[above, font=\footnotesize] at (0.13,0.92) {$ \times 10^{-8}$};
	...
	\begin{tikzpicture}[scale=1]
                \node[anchor=south west,inner sep=0] (image) at (0,0)
        	{
			\includegraphics[width=0.5\textwidth]{figures/P_PR_Rx} 
                };

		\begin{scope}[x={(image.south east)},y={(image.north west)}]
                \node[above, font=\footnotesize] at (0.13,0.92) {$ \times 10^{-11}$};

		\draw[black,thick,<-] (0.56,0.57) --  node[above, font=\footnotesize] {$\ite (1 + \mu)$} (0.76,0.57);
		\draw[black,thick,<-] (0.353,0.57) --  node[above, font=\footnotesize] {$\ite (1 - \mu)$} (0.153,0.57);
                %\draw[help lines,xstep=.1,ystep=.1] (0,0) grid (1,1);
                %\foreach \x in {0,1,...,9} { \node [anchor=north] at (\x/10,0) {0.\x}; }
                %\foreach \y in {0,1,...,9} { \node [anchor=east] at (0,\y/10) {0.\y}; }
                \end{scope}
	...
	\caption{Validationg the theoretical expressions of the pdf and the experimental results with a certain choice of the system parameters depicted in Table \ref{param}.}
	\label{hrel_pdf}
	%\vspace{-10 pt}
\end{figure}
