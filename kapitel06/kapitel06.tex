...
\subsection{Simplifications}
\label{ssec:simp2}
In addition to the simplifications proposed for the hardware validation in Section \ref{ssec:simp2}, following simplifications are considered to transform the analytical framework in the form of the demonstrator: 
...
\begin{figure}
	...
	\vspace{-10pt}
	\caption{A snapshot of the GUI illustrating the performance parameters at the ST and the PR}
	\label{interface}
\end{figure}

At $\mu = 0.05$, it is observed that the demonstrator fails to achieve the target value of the confidence probability constraint $\pcod = 0.95$. Certainly, this issue is largely caused due to the simplifications undertaken in Section \ref{ssec:simp2}. In future, it is important to relax these simplifications. Another possible reason for this behaviour can be speculated as follows: Since a pilot signal produced by a signal generator has been used for the hardware validation, which offers a higher signal quality than the one produced by a USRP in the demonstrator. 
