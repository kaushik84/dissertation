...
The aforementioned analysis is used to determine the suitable estimation time $\ttesto$\footnote{Since the $\ttesto$ is not optimal for all $\snrrcvd$, it is different from $\ttest$ and assigned a different notation.}  for the demonstrator. For a confidence probability constraint $\pcod = 0.95$, $\ttesto = \SI{24}{ms}$ is chosen to be estimation time for the channel estimation, cf. \figurename~\ref{fig:Tausnr}. With this, the interference constraint is satisfied for all $\snrrcvd$ at the cost of a decreased performance in expected secondary throughput, particularly for the situation with high $\snrrcvd$ that achieves the suitable estimation time at lower value.
