\chapter{Hybrid System} \label{chap:HS}
\index{Hybrid system|(}
%\vspace{1.0cm}
%\begin{abstract}
...
\index{Hybrid system!involved channels}
...
As illustrated in Chapter \ref{chap:IS} the primary and the secondary systems employ OFDM\index{OFDM} to carry out their transmissions. As a result, OFDM signals transmitted by the primary system are modelled as zero mean Gaussian signals by the secondary system. 
...
To employ a power control mechanism, the ST is required to control its transmit power in such a way that the interference power received at the PR is below a certain interference threshold ($\ite$)\index{Interference threshold}. In reference to the HS, constraints on interference power received (corresponding to $\pd$ and $1 - \pd$) at the PR are defined as
\begin{align}
...
Next, \index{Secondary throughput}the throughput received at the SR, corresponding to the cases illustrated in (\ref{eq_HS:sig_mod_sr}), is characterized. Subject to the sensing outcome $1 - \pfa, 1 - \pd, \pfa, \pd$, the corresponding throughputs at the SR are defined as 
\begin{align}
...
\end{align}
where $\cz, \co, \ctw$ and $\cth$ represent the data rates\index{Data rate}.

In order to highlight the relationship between the detector's performance (sensing time) and the secondary throughput, which characterize the performance of the HS, Sharma \textit{et al.} \cite{Sharma14} established a sensing-throughput tradeoff\index{Tradeoffs!sensing-throughput tradeoff} subject to a target detection probability $(\pdd)$ and the interference constraint, defined in (\ref{eq_HS:IT_ida}). This tradeoff is represented as
\begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{Hybrid system!frame structure}
...
\subsubsection*{Estimation of Interference Channel ($\hptw$)}
...
Motivated by this fact, the uncertain interference is captured by proposing new outage constraints\index{Interference constraint!outage constraint} $\opdc$ and $\opc$ on estimated detection probability ($\epd$) and estimated power received at PR ($\eprcvdpr$), respectively, as PU constraints for the HS. These constraints are defined as  
\begin{align}
...
As a result, the expected secondary throughput\index{Secondary throughput} is given by 
%\begin{figure*}
\begin{align}
...
In this section, the performance of the proposed approach is evaluated. In this view, simulations are performed: (i) to validate the expressions obtained in the previous section, (ii) to analyze the performance loss incurred due to the time allocation for the estimation and the imperfect channel knowledge. In order to illustrate the performance degradation, the ideal model is considered to benchmark the performance of the proposed approach. Unless stated explicitly, the choice of parameters given in Table \ref{tb_HS:tb2} is considered for analysis. 
