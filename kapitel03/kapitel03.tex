...
The analysis performed in this chapter reveals that with an appropriate choice of the estimation time and the sensing time can effectively control the performance degradation of the IS, which subsequently yield the achievable secondary throughput.
...
Analog to the sensing channel, the SR performs received power-based estimation by listening to the transmission from the PT. The knowledge of $\hpt$ is required to characterize the interference from the PT. Under $\mathcal H_0$, the discrete signal model at the SR is given as 
\begin{align}
