...
Under practical scenarios, it has been well-recognized that the knowledge of related channels is crucial for the application of CR techniques. With an intention of facilitating the hardware deployment of a CSC, a CR application, this thesis capitalizes on the integration of this knowledge for different CR systems, namely interweave systems, underlay system and hybrid systems. The main contributions and the observations of this thesis are summarized as follows:
\begin{itemize}
\item \textit{Analytical Framework}: 
As a major contribution, this thesis proposes an analytical framework that facilitates a successful incorporation of the estimation of the channels involved between the primary and the secondary systems corresponding to different CR systems (in chapter \ref{chap:IS}, chapter \ref{chap:US} and chapter \ref{chap:HS}, respectively). In order to emphasize on the deployment, this framework proposes a received power based channel estimation that satisfies the low complexity and versatility towards unknown PU signal requirements. As a part of this framework, a certain time allocation for the channel estimation, corresponding to the medium access, is proposed. Besides, a probabilistic approach of handling the variation in the CR system (estimation error) arising due to the imperfect channel knowledge is established. In contrast to the existing models that precludes channel estimation (or consider perfect channel knowledge), thereby overestimating the performance of the CR systems, this thesis provides insights on the influence of the time allocation and imperfect channel knowledge that arises due to channel estimation on the performance of the CR system. This influence, which leads to a \textit{performance degradation}, is qualified through a comparison between the existing models and the estimation model, proposed as a part of this framework. Particularly, the variations that may cause excessive interference to the primary system, which otherwise could lead to serious interference, are captured by proposing novel constraints within this framework. In order to examine the relationship between the system parameters, theoretical expressions pertaining to the performance analysis of the CR system are derived. To further quantify the performance of the prosed framework in fading scenarios, especially the interweave and the underlay systems, the framework is extended to derive theoretical expressions that including channel fading. Finally, to avoid any discrepancy in the performance analysis, these expressions are validated by means of Monte-Carlo simulations. 
\item \textit{Tradeoff}: 
