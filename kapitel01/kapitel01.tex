...
In context with a CR system, the channel knowledge, unlike conventional or state-of-the-art wireless systems, is not confined to a single transmitter-receiver link, rather it includes all the related channels that exist within as-well-as across the primary and the secondary systems, cf. \figurename~\ref{fig:scenario}. Now, since the performance of CR system jointly depends on these two systems, the channel knowledge is a paramount for the performance characterization. In absence of this knowledge, especially of those channels that are related to the interference at the primary systems render the performance characterization of a CR system incomplete and inappropriate for practical implementations. Despite the existence of multitude of analytical models in the literature that consider with the performance analysis of a CR, its performance with regard to the channel estimation, due to the complexity of the problem, has never been completely understood. Curtailing this gap, this thesis capitalizes on the estimation of the involved channel in the CR systems. In other words, making the channel knowledge accessible at the CSC-BS, enables the implementation of cognitive techniques (as interweave, underlay and hybrid systems) at the CSC-BS, and utilize the spectrum to establish a communication link with the MS, without exceeding the desired level of interference at the PR. 

The inclusion of channel estimation demands an allocation of a certain time interval by the CSC-BS. In consideration to this allocation, a certain degradation in the performance in terms of throughput is obvious. Besides, the variations introduced due to the estimation process, also treated as imperfect channel knowledge, may cause an \textit{excessive interference}\footnote{The excessive here symbolizes the interference that exists only because of the imperfect channel knowledge, which is not dealt in the literature.} to the primary systems. If not considered, this excessive interference may severely degrade the performance of the CR systems. In order to facilitate the incorporation of channel estimation into the CR system, it is essential to consider these effects in the analytical model, which otherwise are non-existent in the systems models that considers the perfect knowledge of the involved channels. Taking these effects into account, the established analytical framework allows us to understand the performance of the CR system under situations that are much closer to realistic scenarios. Following these arguments, an analytical framework subject to the underlying CR systems is proposed. This proposed framework is further utilized to characterize the performance of the respective CR systems. 

Coming back to the deployment-centric viewpoint, it is worthy to understand that the secondary access to the licensed spectrum is viable only if the CR system aligns itself to the primary system. This signifies its dependency on the wireless standard followed by the primary system, which enables it to carry out the preliminary processing (that includes the synchronization and demodulation) of the baseband signal received from the primary system. The existence of multiple wireless standards and their complexity forbid us to deploy a dedicated circuitry for each primary system \cite{Ghasemi08_cm}. Under this notion, only those solutions that are allows low complex and versatility to different primary user signal are widely accepted by the community, for instance energy based detection employed by the interweave systems (discussed later in chapter \ref{chap:IS}) to detect the primary user signal is a largely accepted technique compare to other techniques such as matched-filtering and cyclo-stationary based detection. Pawelczak \textit{et. al.} \cite{Pawe11} compares these techniques by counting their implementations for hardware demonstration. 

On similar grounds, in order to approach the channel estimation for the CR system, particularly for the channels that involve the primary systems, in contrast to conventional techniques such as pilot-based channel estimation, a receive power-based channel estimation technique is proposed as a part of the analytical framework in the thesis. By doing this, the low-complexity and versatility to the unknown primary user signal is assured. This again facilitates the deployment of the CR system. At the other end, for the channel within the secondary framework, a pilot-based channel estimation is employed.
