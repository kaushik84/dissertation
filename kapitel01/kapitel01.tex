...
Under practical scenarios, it has been well-recognized that the knowledge of related channels is crucial for the application of CR techniques. With an intention of facilitating the hardware deployment of a CSC, a CR application, this thesis capitalizes on the successful integration of this knowledge for different CR systems, namely interweave systems, underlay system and hybrid systems. The main contributions and the observations of this thesis are summarized as follows:
\begin{itemize}
\item \textit{Analytical Framework}: 
As a major contribution, this thesis proposes an analytical framework, corresponding to different CR systems (in chapter \ref{chap:IS}, chapter \ref{chap:US} and chapter \ref{chap:HS}, respectively), that incorporates the estimation of the channels involved between the primary and the secondary systems. This framework proposes a received power based channel estimation that satisfies the low complexity and versatility towards unknown PU signal requirements, necessary for the deployment of the CR systems. As a part of this framework, a certain time allocation for the channel estimation, corresponding to the medium access, is proposed. Besides, a probabilistic approach of handling the variation in the CR system (estimation error) arising due to the imperfect channel knowledge is established. In contrast to the existing models that precludes channel estimation (or consider perfect channel knowledge), thereby overestimating the performance of the CR systems, this thesis provides insights on the impact of time allocation and imperfect channel knowledge that arises due to channel estimation on the performance of the CR system. This impact leads to degradation in the performance of the CR systems is quantified by outlining a comparison between the existing models and the estimation model (proposed as a part of this framework). Particularly, the variations that may causes excessive interference to the primary system, which otherwise could lead to serious interference, are captured by means of new constraints proposed within this framework. For the interweave and underlay systems, the proposed framework is extended to analyze its performance subject to channel fading. Finally, to carefully understand the dependency on the performance of different parameters, theoretical expressions of the performance parameters are derived. In order to avoid any discrepancy in the analysis, these expressions are validated by means of Monte-Carlo simulations. 
\item \textit{Tradeoff}: 
The estimation time has an direct influence on the performance of the CR systems.
